...
export function getDemoOnboarding(): OnboardingData {
  ...
  },
  {
    ...
    readMinutes: 12,
    body: `6.1. СБОР ИНФОРМАЦИИ — 90% УСПЕХА\n\nЧто узнать о клиенте:\n• период существования, направления бизнеса\n• выручка, прибыль\n• численность животных, порода\n• кто принимает решения (ЛПР)\n• с кем работают сейчас, какие препараты используют\n• способ закупки (тендер, прямой договор, дистрибьютор)\n\n6.2. УСТАНОВЛЕНИЕ КОНТАКТА\n\nЦель: договориться о встрече.\n\n6.3. ПРЕЗЕНТАЦИЯ И ВЫЯВЛЕНИЕ ПОТРЕБНОСТЕЙ\n\nТехника СПИН: ситуационные, проблемные, извлекающие, направляющие вопросы.\n\n6.4. РАБОТА С ВОЗРАЖЕНИЯМИ\n\n| Возражение | Ответ |\n| --- | --- |\n| «Дорого» | «Давайте посчитаем потери. Курс Миомага — 63 руб., а потери — более 500 тыс. руб.» |\n| «Работаем с другим» | «Предлагаю пробу на 10 головах.» |\n| «Нет денег» | «Обсудим отсрочку или тестовую партию.» |`
  },
    {
    id: '54000000-0000-4000-8000-000000000103',
    title: 'Карта поддержки и рабочие системы',
    summary: 'Корпоративные каналы, ключевые операции в 1С, контакты подразделений и порядок эскалации.',
    category: 'Продажи · Процессы',
    readMinutes: 11,
    body: `ОСНОВНЫЕ СИСТЕМЫ:\n\n• Корпоративная почта — приём заказов, письменные подтверждения, отправка документов. Заказы принимаются на sale@bionit.ru.\n• 1С ERP — создание заказов, проверка условий продаж, контроль готовности, оформление реализации.\n• Битрикс24 — задачи, файлы, внутренняя коммуникация.\n• CRM — клиенты, сделки, история взаимодействий.\n\nКЛЮЧЕВЫЕ ОПЕРАЦИИ В 1С:\n\n| Задача | Путь или действие |\n| --- | --- |\n| Создать заказ клиента | «Продажи» → «Заказы клиентов» → «Создать» |\n| Проверить оплату | «Продажи» → «Расчёты с клиентами» → «Оплата клиентов» |\n| Проверить готовность заказа | «Продажи» → «Доп. обработки» → «Приоритеты отгрузок» |\n| Оформить реализацию (УПД) | «Продажи» → «Накладные к оформлению» → «Оформить по заказам» |\n| Контролировать дебиторскую задолженность | «Продажи» → «Отчёты по продажам» → «Дебиторская задолженность» |\n\nКОНТАКТЫ:\n\n| Роль | Контакт |\n| --- | --- |\n| Офис | +7 (4922) 34-16-21 |\n| Бухгалтерия | Елена Хуртова, +7 960 719-70-70 |\n| Главный бухгалтер | Екатерина Алехина, +7 915 772-78-00 |\n| Логист | Татьяна Кравчук, +7 903 833-58-33 |\n| Склад Лакина | Екатерина Капранова, +7 904 596-32-56 |\n| Склад Мостостроевская | Иван Бирюков, +7 900 474-67-78 |\n| Программист 1С | Владимир Катков, +7 903 647-01-95 |\n| Руководитель | Валерий Распопов, +7 (926) 215-08-83 |\n\nСТАТУСЫ ЗАКАЗА:\n• «На согласовании» — по умолчанию. Заказ не виден производству.\n• «К выполнению» — после письменного подтверждения клиентом или предоплаты.\n\nПРАВИЛО ЭСКАЛАЦИИ: передавайте не «не работает», а: клиент, номер заказа, ожидаемый результат, фактический результат, скриншот, срочность.`
  },
    {
    id: '54000000-0000-4000-8000-000000000104',
    title: 'Первые 14 дней: контрольные точки',
    summary: 'Практический маршрут по основной воронке продаж: от заказа до отгрузки, оплаты и документов.',
    category: 'Продажи · Воронка',
    readMinutes: 15,
    body: `ДЕНЬ 1 — ПРИНЯТЬ И ВВЕСТИ ЗАКАЗ\n\n• Заказы принимаются на sale@bionit.ru\n• Ввод в 1С: «Продажи» → «Заказы клиентов» → «Создать»\n• Статус по умолчанию: «На согласовании»\n• После подтверждения: «К выполнению»\n\nКонтрольная точка Дня 1:\n• письмо клиента сохранено\n• заказ создан в 1С\n• позиции, объёмы и цены проверены\n• понятен следующий шаг\n\nДЕНЬ 3 — СОГЛАСОВАТЬ ЦЕНУ И ОПЛАТУ\n\n• Торги: не предлагать минимальную цену сразу, снижать поэтапно\n• Для постоплаты: письменное подтверждение по email\n• Для предоплаты: дождаться поступления аванса\n\nКонтрольная точка Дня 3:\n• цена и объём согласованы\n• подтверждение или предоплата получены\n• заказ в статусе «К выполнению»\n\nДЕНЬ 7 — ПРОВЕРИТЬ ГОТОВНОСТЬ И ЛОГИСТИКУ\n\n• Отчёт: «Продажи» → «Доп. обработки» → «Приоритеты отгрузок»\n• Способы доставки и минимальные суммы:\n\n| Способ поставки | Мин. сумма без НДС |\n| --- | --- |\n| Доставка за счёт компании | От 150 000 руб. |\n| Самовывоз | От 100 000 руб. |\n| Вакцины (исключение) | От 1 блистера* |\n\n*для клиентов, купивших с начала года от 100 000 руб.\n\n• Контроль сроков годности: ОСГ не менее 70%\n• Для продуктов с 12-месячным сроком — порог через 3,5 месяца\n• Терморежим: вакцины +2…+8°C, фарма +5…+25°C\n\nКонтрольная точка Дня 7:\n• прогноз готовности передан клиенту\n• способ доставки согласован\n• ОСГ и терморежим проверены\n\nДЕНЬ 14 — ОФОРМИТЬ ОТГРУЗКУ\n\n• Реализация: «Продажи» → «Накладные к оформлению» → «Оформить по заказам»\n• Счёт-фактура: «Оформить счёт-фактуру» → «Провести и закрыть»\n• Постотгрузка: через 1-2 дня связаться с клиентом, отправить накладные\n• Контроль оплаты: «Дебиторская задолженность по клиентам»\n• Оригиналы документов: контролировать возврат\n\nКонтрольная точка Дня 14:\n• реализация и счёт-фактура оформлены\n• клиент подтвердил получение\n• срок оплаты контролируется\n• возврат оригиналов на контроле`
  }
],
questions: [
