...
export function submitDemoLearningAttempt(
  ...
  const correctCount = questionIds.reduce(
    (total, questionId) =>
      total + (answers[questionId] === correctAnswers[questionId] ? 1 : 0),
    0
  );
...
/**
 * Совместимые имена экспортов для текущей архитектуры проекта.
 * Загрузчики и API-маршруты используют префикс getDemo*, а внутренние
 * реализации сохранены под исходными именами, чтобы не ломать другие импорты.
 */
...
export function getDemoLearningClass(classId: string): LearningClassDetail {
  const learningClass = demoLearningDetail(classId);

  if (!learningClass) {
    throw new Error('Учебный класс не найден.');
  }
